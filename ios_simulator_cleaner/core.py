"""Core functionality for ios-simulator-cleaner CLI."""
...
def list_simulators() -> List[Dict[str, Any]]:
  """List simulators found on disk and display them in a table."""
  ...
  with console.status("[bold green]Scanning simulators...[/bold green]", spinner="dots"):
    for device_dir in SIMULATORS_PATH.iterdir():
      ...
      if plist_file.exists():
        try:
          with plist_file.open('rb') as file_handle:
          ...
          console.print(f"[red]Error reading {device_dir.name}: {exc}[/red]")

  if not simulators:
    console.print("[yellow]No simulators found.[/yellow]")
    return []

  simulators.sort(key=lambda sim: sim["sort_key"])
...
def delete_simulator(sim_id: str) -> None:
  """Delete the simulator with the given identifier."""
...
def delete_all_simulators() -> None:
  """Delete every simulator after user confirmation."""
...
def list_previews() -> float:
  """List SwiftUI previews and return the total size in GB."""
...
def delete_previews() -> None:
  """Delete the SwiftUI previews directory."""
...
def main_menu() -> None:
  """Interactive menu that drives the CLI workflow."""
  while True:
      ...
      if sim_action == "List":
        ...
        if sims:
          ...
          confirm = questionary.confirm(
            f"⚠️ Are you absolutely sure you want to delete simulator '{sim_id}'?",
            default=False,
          ).ask()
          if confirm:
        ...
        confirm = questionary.confirm(
          "Are you sure you want to delete ALL simulators?"
        ).ask()
        if confirm:
          double_check = questionary.confirm(
            "⚠️ This action is irreversible. Do you really want to proceed?",
            default=False,
          ).ask()
          if double_check:
